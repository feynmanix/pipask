import pytest
from unittest.mock import AsyncMock, MagicMock

from pipask.checks.package_downloads import check_package_downloads
from pipask.checks.types import CheckResultType
from pipask.infra.pypistats import PypiStatsClient, DownloadStats
from pipask.infra.pip import InstallationReportItem, InstallationReportItemMetadata, InstallationReportItemDownloadInfo

PACKAGE_NAME = "package"
PACKAGE_VERSION = "1.0.0"
REPORT_ITEM = InstallationReportItem(
    metadata=InstallationReportItemMetadata(name=PACKAGE_NAME, version=PACKAGE_VERSION),
    download_info=InstallationReportItemDownloadInfo(url="https://example.com"),
    requested=True,
    is_yanked=False,
)


@pytest.mark.asyncio
async def test_package_downloads_no_stats():
    pypi_stats_client = MagicMock(spec=PypiStatsClient)
    pypi_stats_client.get_download_stats = AsyncMock(return_value=None)

    result = await check_package_downloads(REPORT_ITEM, pypi_stats_client)

    assert result.result_type == CheckResultType.WARNING
    assert result.message == "No download statistics available"


@pytest.mark.asyncio
async def test_high_download_count():
    pypi_stats_client = MagicMock(spec=PypiStatsClient)
    pypi_stats_client.get_download_stats = AsyncMock(
        return_value=DownloadStats(last_month=5000, last_week=500, last_day=50)
    )

    result = await check_package_downloads(REPORT_ITEM, pypi_stats_client)

    assert result.result_type == CheckResultType.SUCCESS
    assert result.message == "5,000 downloads from PyPI in the last month"


@pytest.mark.asyncio
async def test_low_download_count():
    pypi_stats_client = MagicMock(spec=PypiStatsClient)
    pypi_stats_client.get_download_stats = AsyncMock(
        return_value=DownloadStats(last_month=500, last_week=10, last_day=0)
    )

    result = await check_package_downloads(REPORT_ITEM, pypi_stats_client)

    assert result.result_type == CheckResultType.WARNING
    assert result.message == "Only 500 downloads from PyPI in the last month"
